...
# Expected: {"status":"ok","app":"Solarhesap","version":"v0.3.0"}
```
...
## 9. Updating the Application (Deploying New Code)

When a new version is pushed to GitHub, you can update your live server seamlessly without manually stopping any containers. Docker Compose will automatically handle the graceful recreation of updated services.
...
# 3. Rebuild and restart the containers in the background
docker compose -f docker-compose.yml up -d --build
```

> **Note:** The `-d` flag keeps it running in the background, and `--build` ensures that any new dependencies (in `package.json` or `requirements.txt`) are installed and new CSS/JS is compiled.

To rebuild only a specific service (e.g., just the backend):
