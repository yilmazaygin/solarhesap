...
def resolve_strategies(strategies: list[str]) -> list[str]:
    """Return a de-duplicated strategy list."""
...
class ClearskyWithDatesRequest(ClearskyBaseRequest):
    ...
    avg_year_strategies: list[str] = Field(
        ...
        description=(
            ...
            "simple_mean, trimmed_mean, exponential_weighted"
        ),
    )
...
class DeepComparisonYieldRequest(DeepComparisonRequest):
    """Deep comparison for electric energy yield."""
    efficiency_tier: str = Field(default="medium", description="Panel efficiency tier")
    area_m2: float | None = Field(default=None, gt=0, description="Area in m²")
    area_a: float | None = Field(default=None, gt=0, description="Dimension A")
    area_b: float | None = Field(default=None, gt=0, description="Dimension B")
...
class RunModelChainRequest(BaseModel):
    ...
    avg_year_strategies: list[str] = Field(
        ...
        description=(
            "Which avg-year strategies to compute. Accepts: "
            "simple_mean, trimmed_mean, exponential_weighted. "
            "Ignored when weather_source is 'pvgis_tmy'."
        ),
    )
    decay: float = Field(default=0.90, gt=0, lt=1, description="Exponential decay factor (0–1)")
    lower_percentile: float = Field(default=10.0, ge=0, le=50, description="Trimmed-mean lower %")
