...
def run_electric_simulation_from_df(req: Any, weather_df: pd.DataFrame, meta_note: str) -> dict[str, Any]:
    """Run electric production simulation using the provided weather DataFrame.
    
    `req` must have: efficiency_tier, area_m2, area_a, area_b, latitude, longitude, elevation,
    ...
    alogger.info(
        "ElectricSim: tier=%s  area=%.1f m²  panels=%d  strings=%d  "
    ...
    if layout["dc_ac_warning"]:
        alogger.warning("ElectricSim DC/AC: %s", layout["dc_ac_warning"])
    ...
    if "poa_global" in weather_df.columns:
...
def run_basic_electric(req: BasicElectricRequest) -> dict[str, Any]:
    ...
    meta_note = "Year-1 estimate. Apply 0.5-0.7 %/yr panel degradation for multi-year projections."
    res = run_electric_simulation_from_df(req, tmy_df, meta_note)
    res["metadata"]["weather_source"] = "PVGIS TMY (SARAH3 2005-2023 + ERA5)"
    res["metadata"]["months_selected"] = meta.get("months_selected", [])
    return res
