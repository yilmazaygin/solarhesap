...
import { Copy, Check, ChevronDown, ChevronRight, Zap, Wrench, Activity, BookOpen, Code2, Cpu, Globe, Server } from "lucide-react";
...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      {
        method: "POST",
        path: "/solar-simulation/deep-comparison-yield",
        summary_en: "Deep Comparison for Electric Yield",
        summary_tr: "Elektrik Üretimi için Derin Karşılaştırma",
        desc_en: "Similar to deep comparison but includes module/inverter configuration to directly compare AC electric yield across all models.",
        desc_tr: "Derin karşılaştırmaya benzer ancak tüm modellerde doğrudan AC elektrik üretimini karşılaştırmak için modül/inverter konfigürasyonunu dahil eder.",
        fields: [
          { name: "latitude", type: "float", required: true, desc_en: "Latitude", desc_tr: "Enlem" },
          { name: "longitude", type: "float", required: true, desc_en: "Longitude", desc_tr: "Boylam" },
          { name: "inverter", type: "object", required: true, desc_en: "Inverter configuration", desc_tr: "İnverter yapılandırması" },
          { name: "flat_system", type: "object", required: false, desc_en: "Flat system config", desc_tr: "Düz sistem yapılandırması" },
          { name: "use_arrays", type: "bool", required: false, default: "false", desc_en: "Use arrays instead of flat_system", desc_tr: "flat_system yerine arrays kullan" },
          { name: "arrays", type: "array", required: false, desc_en: "Array configs", desc_tr: "Dizi yapılandırmaları" },
          { name: "start_year / end_year", type: "int", required: false, default: "2015 / 2020", desc_en: "Analysis range", desc_tr: "Analiz aralığı" },
          { name: "models", type: "string[]", required: false, default: '["instesre_bird","ineichen","simplified_solis","pvlib_bird"]', desc_en: "Models to include", desc_tr: "Dahil edilecek modeller" },
          { name: "include_pvgis_tmy", type: "bool", required: false, default: "true", desc_en: "Include PVGIS TMY", desc_tr: "PVGIS TMY'yi dahil et" },
        ],
        example: { latitude: 39.93, longitude: 32.86, start_year: 2015, end_year: 2020, use_arrays: false, inverter: { "source": "database", "db_name": "CECInverter", "inverter_name": "ABB__MICRO_0_25_I_OUTD_US_208__208V_" }, flat_system: { "surface_tilt": 30, "surface_azimuth": 180, "module": { "source": "database", "db_name": "CECMod", "module_name": "Canadian_Solar_Inc__CS6K_275P" } } },
      },
    ],
  },
  {
  ...
  },
  {
    ...
    endpoints: [
      {
        method: "POST",
        path: "/solar-tools/julian-day",
        summary_en: "Julian Day Number",
        summary_tr: "Jülyen Gün Numarası",
        desc_en: "Compute the Julian Day Number from a calendar date or ISO string.",
        desc_tr: "Takvim tarihi veya ISO dizesinden Jülyen Gün Numarasını hesaplar.",
        fields: [
          { name: "year / month / day", type: "int", required: true, desc_en: "Date", desc_tr: "Tarih" },
          { name: "hour / minute / second", type: "int", required: false, default: "0 / 0 / 0", desc_en: "Time", desc_tr: "Zaman" },
        ],
      ...
      {
        method: "POST",
        path: "/solar-tools/solar-declination",
        summary_en: "Solar Declination",
        summary_tr: "Güneş Sapması",
        desc_en: "Calculate solar declination angle for a given date using Spencer's Fourier series.",
        desc_tr: "Spencer'ın Fourier serisini kullanarak belirli bir tarih için güneş sapma açısını hesaplar.",
        fields: [
          { name: "year / month / day", type: "int", required: true, desc_en: "Date", desc_tr: "Tarih" },
        ],
        example: { year: 2024, month: 6, day: 21 },
      ...
      {
        method: "POST",
        path: "/solar-tools/dew-point-to-pw",
        summary_en: "Dew Point to Precipitable Water",
        summary_tr: "Çiğ Noktasından Yağış Suyu",
        desc_en: "Convert dew point temperature (°C) to precipitable water (cm) using the Leckner (1978) formula.",
        desc_tr: "Çiğ noktası sıcaklığını (°C) Leckner (1978) formülü kullanarak yoğunlaşabilir suya (cm) dönüştürür.",
        fields: [
          { name: "dew_point", type: "float", required: true, desc_en: "Dew point temperature (°C)", desc_tr: "Çiğ noktası sıcaklığı (°C)" },
        ],
        example: { dew_point: 10.5 },
      },
      {
        method: "POST",
        path: "/solar-tools/station-pressure",
        summary_en: "Station Pressure from Elevation",
        summary_tr: "Yükseklikten İstasyon Basıncı",
        desc_en: "Estimate station-level atmospheric pressure from elevation and sea-level pressure using the barometric formula.",
        desc_tr: "Barometrik formülü kullanarak yükseklik ve deniz seviyesi basıncından istasyon seviyesi atmosferik basıncını tahmin eder.",
        fields: [
          { name: "elevation", type: "float", required: true, desc_en: "Elevation in metres", desc_tr: "Yükseklik (m)" },
          { name: "sea_level_pressure", type: "float", required: false, default: "101325.0", desc_en: "Sea-level pressure in Pascals", desc_tr: "Deniz seviyesi basıncı (Pascal)" },
        ],
        example: { elevation: 900, sea_level_pressure: 101325.0 },
      },
      {
        method: "POST",
        path: "/solar-tools/isa-pressure",
        summary_en: "ISA Standard Atmosphere Pressure",
        summary_tr: "ISA Standart Atmosfer Basıncı",
        desc_en: "Calculate pressure and temperature at elevation using the International Standard Atmosphere model (troposphere only).",
        desc_tr: "Uluslararası Standart Atmosfer modelini (yalnızca troposfer) kullanarak yükseklikteki basınç ve sıcaklığı hesaplar.",
        fields: [
          { name: "elevation", type: "float", required: true, desc_en: "Elevation in metres", desc_tr: "Yükseklik (m)" },
        ],
        example: { elevation: 900 },
      },
      {
        method: "POST",
        path: "/solar-tools/linke-turbidity",
        summary_en: "Linke Turbidity Estimate",
        summary_tr: "Linke Türbidite Tahmini",
        desc_en: "Estimate Linke turbidity factor from precipitable water and aerosol optical depth at 700 nm.",
        desc_tr: "Yoğunlaşabilir su ve 700 nm'deki aerosol optik derinliğinden Linke türbidite faktörünü tahmin eder.",
        fields: [
          { name: "precipitable_water", type: "float", required: true, desc_en: "Precipitable water (cm)", desc_tr: "Yoğunlaşabilir su (cm)" },
          { name: "aod700", type: "float", required: true, desc_en: "Aerosol Optical Depth at 700 nm", desc_tr: "700 nm'de Aerosol Optik Derinliği" },
          { name: "airmass", type: "float", required: false, default: "2.0", desc_en: "Relative optical air mass", desc_tr: "Göreli optik hava kütlesi" },
        ],
        example: { precipitable_water: 2.5, aod700: 0.1, airmass: 2.0 },
      },
      {
        method: "POST",
      ...
      {
        method: "POST",
        path: "/solar-tools/instant-bird",
        summary_en: "Instant Bird Clearsky",
        summary_tr: "Anlık Bird Açık Gökyüzü",
        desc_en: "Single-point INSTESRE Bird clearsky calculation. Returns GHI, DNI, DHI plus solar position for a single instant.",
        desc_tr: "Tek noktalı INSTESRE Bird açık gökyüzü hesaplaması. Tek bir an için GHI, DNI, DHI ve güneş konumunu döndürür.",
        fields: [
          { name: "latitude", type: "float", required: true, desc_en: "Latitude", desc_tr: "Enlem" },
          { name: "longitude", type: "float", required: true, desc_en: "Longitude", desc_tr: "Boylam" },
          { name: "elevation", type: "float", required: false, default: "0.0", desc_en: "Elevation (m)", desc_tr: "Yükseklik (m)" },
          { name: "year / month / day / hour / minute", type: "int", required: true, desc_en: "UTC Timestamp", desc_tr: "UTC Zaman damgası" },
          { name: "ozone / aod500 / aod380 / albedo", type: "float", required: false, desc_en: "Atmospheric parameters", desc_tr: "Atmosferik parametreler" },
        ],
        example: { latitude: 39.93, longitude: 32.86, year: 2024, month: 6, day: 21, hour: 12, minute: 0 },
      },
      {
        method: "POST",
...
export default function ApiDocsPage() {
  ...
  return (
    <main className="min-h-screen page-bg pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        <div className="mb-10 animate-fade-in flex flex-col items-center justify-center text-center gap-4 relative">
          <div className="flex flex-col items-center gap-2">
            <h1 className="text-3xl sm:text-5xl font-bold tracking-tight flex items-center justify-center gap-3">
        ...
        <div className="glass-card space-y-4 animate-slide-up">
          ...
          <div className="p-3 rounded-lg bg-emerald-500/[0.06]" style={{ border: "1px solid rgba(34,197,94,0.2)" }}>
            ...
            <pre className="text-xs font-mono whitespace-pre-wrap break-all sm:break-normal" style={{ color: "var(--text-secondary)" }}>{healthCurl}</pre>
          </div>
        </div>

        {/* ── Features ── */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 animate-slide-up" style={{ animationDelay: "100ms" }}>
          <div className="glass-card p-5 flex flex-col items-center text-center gap-3 relative overflow-hidden group">
            ...
            <Cpu className="h-6 w-6 text-amber-400 relative z-10" />
            <h3 className="text-sm font-bold text-slate-100 relative z-10">{isTr ? "Bilimsel Doğruluk" : "Scientific Accuracy"}</h3>
            <p className="text-xs text-slate-400 leading-relaxed relative z-10">
              {isTr ? "PVLib altyapısı ve PVGIS entegrasyonu sayesinde endüstri standartlarında güvenilir hesaplamalar." : "Industry-standard reliable calculations powered by PVLib infrastructure and PVGIS integration."}
            </p>
          </div>
          <div className="glass-card p-5 flex flex-col items-center text-center gap-3 relative overflow-hidden group">
            ...
            <Server className="h-6 w-6 text-emerald-400 relative z-10" />
            <h3 className="text-sm font-bold text-slate-100 relative z-10">{isTr ? "Yüksek Performans" : "High Performance"}</h3>
            <p className="text-xs text-slate-400 leading-relaxed relative z-10">
              {isTr ? "FastAPI tabanlı asenkron backend mimarisi ile milisaniyeler içerisinde yanıt veren güçlü altyapı." : "Powerful infrastructure responding in milliseconds with FastAPI-based asynchronous backend architecture."}
            </p>
          </div>
          <div className="glass-card p-5 flex flex-col items-center text-center gap-3 relative overflow-hidden group">
            ...
            <h3 className="text-sm font-bold text-slate-100 relative z-10">{isTr ? "Geliştirici Dostu" : "Developer Friendly"}</h3>
            <p className="text-xs text-slate-400 leading-relaxed relative z-10">
              {isTr ? "Tamamen açık ve dökümante edilmiş RESTful uç noktalarıyla projenize kolayca entegre edin." : "Easily integrate into your projects with fully open and documented RESTful endpoints."}
            </p>
          </div>
        </div>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 px-2 py-3 rounded-xl text-[10px] sm:text-xs font-medium transition-all text-center leading-tight border ${
                    !isActive ? "hover:bg-white/[0.04]" : ""
                  }`}
                  style={isActive
                    ? { background: "var(--accent)", color: "#fff", borderColor: "var(--accent)" }
                    : { color: "var(--text-muted)", background: "var(--bg-subtle-2)", borderColor: "var(--border)" }}
                >
