...
import { FileDown, BarChart3, TrendingUp, Eye, EyeOff, Activity, AreaChart as AreaChartIcon } from "lucide-react";
...
export default function ComparisonChart({ comparisonData, summaryMatrix, defaultMode, mode = "irradiance" }: ComparisonChartProps) {
  ...
  const [tsChartType, setTsChartType] = useState<"bar" | "line" | "area">("line");
  ...
  const [selectedMetric, setSelectedMetric] = useState<string>(mode === "yield" ? "ac_kw" : "ghi");
  ...
  const availableMetrics = mode === "yield" ? ["ac_kw"] : ["ghi", "dni", "dhi"];
  ...
  const handleChartClick = (state: any) => {
    ...
    if (drillLevel === "annual" && clickId !== undefined) {
      ...
      setDrillLevel("month");
      setTsChartType("line"); // Default for Daily view
    } else if (drillLevel === "month" && clickId !== undefined) {
      setSelectedDay(clickId);
      setDrillLevel("day");
      setTsChartType("line"); // Default for Hourly
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-bold text-slate-200 flex items-center gap-2">
            ...
            {mode === "yield" 
              ? (language === "tr" ? "AC Üretim Kıyaslaması (kW)" : "AC Production Comparison (kW)")
