...
export default function EnergyDrillChart({ hourlyData, annualKwh }: EnergyDrillChartProps) {
  ...
  const currentTotal = level === "year" 
    ? annualKwh 
    : chartData.reduce((acc, curr) => acc + curr.value, 0);

  const currentTotalStr = currentTotal >= 1000 
    ? (currentTotal / 1000).toFixed(1) + " MWh" 
    : Math.round(currentTotal) + " kWh";

  const crumb =
    level === "year"
      ? `${currentTotalStr} / ${s.year}`
      : level === "month" && selMonth !== null
        ? `${currentTotalStr} / ${MONTH_LONG[selMonth]}`
        : selMonth !== null && selDay !== null
          ? `${currentTotalStr} / ${selDay} ${MONTH_LONG[selMonth]}`
          : "";
