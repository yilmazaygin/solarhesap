  ...
  return (
    <div className="rounded-xl border border-white/[0.1] bg-slate-900/95 backdrop-blur-xl p-3 shadow-xl z-50">
      ...
      {payload.map((entry: any, i: number) => {
        ...
        const labelText = (strat === "simple_mean" || strat === "tmy")
          ? (t("AC Üretim") || (t("AC Production") || "AC Production")) // fallback to language aware manual
          : (t(`constants.strategies.${strat}.label` as any) || strat);
        return (
          <div key={i} className="flex items-center gap-2 text-xs">
            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />
            <span className="text-slate-400">{(strat === "simple_mean" || strat === "tmy") ? (t("language") === "tr" ? "AC Üretim" : "AC Production") : labelText}:</span>
...
export default function ModelChainChart({ title, simulation_results }: ModelChainChartProps) {
  ...
  const strategyTotals = useMemo(() => {
    ...
    strategies.forEach((strat) => {
      ...
      totals[strat] = Object.entries(ac).reduce((sum: number, [dt, v]) => {
    ...
    return totals;
  }, [strategies, simulation_results, drillLevel, selectedMonth, selectedDay]);

  const unit = drillLevel === "day" ? (language === "tr" ? "kWh/gün" : "kWh/day") : drillLevel === "month" ? (language === "tr" ? "kWh/ay" : "kWh/month") : (language === "tr" ? "kWh/yıl" : "kWh/year");
  const periodLabel = drillLevel === "annual" ? (language === "tr" ? "Yıllık" : "Annual") : drillLevel === "month" ? (language === "tr" ? "Aylık" : "Monthly") : (language === "tr" ? "Günlük" : "Daily");
  ...
  return (
    <div className="glass-card" id="modelchain-chart">
        ...
        <div className="flex items-center gap-3">
          ...
          {strategies.length === 1 && (
            <span className="text-xs text-slate-400 font-medium whitespace-nowrap hidden sm:block">
              {strategyTotals[strategies[0]] < 100 ? strategyTotals[strategies[0]].toFixed(2) : Math.round(strategyTotals[strategies[0]]).toLocaleString()} {unit}
            </span>
          )}
          {deltaMode && visibleStrats.length > 1 && (
            <span className="text-[10px] uppercase font-bold text-cyan-400 bg-cyan-400/10 px-2 py-0.5 rounded ml-2">
              Delta: {(visibleStrats[0] === "simple_mean" || visibleStrats[0] === "tmy") ? (language === "tr" ? "AC Üretim" : "AC Production") : (t(`constants.strategies.${visibleStrats[0]}.label` as any) || visibleStrats[0])}
            </span>
          )}
        </div>
      ...
      {/* Strategies Legend - Only show if multiple strategies exist */}
      {strategies.length > 1 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {strategies.map((strat, i) => {
            const isVisible = !hiddenStrats.has(strat);
            const label = (strat === "simple_mean" || strat === "tmy") ? (language === "tr" ? "AC Üretim" : "AC Production") : (t(`constants.strategies.${strat}.label` as any) || strat);
            return (
              <button
                key={strat}
                type="button"
      ...
      {/* Totals summary - Only show if multiple strategies exist */}
      {strategies.length > 1 && (
        <div className="mb-6 flex flex-wrap gap-3">
          {strategies.map((strat, i) => {
            const total = strategyTotals[strat];
            const label = (strat === "simple_mean" || strat === "tmy") ? (language === "tr" ? "Toplam AC Üretim" : "Total AC Production") : (t(`constants.strategies.${strat}.label` as any) || strat);
            const isHidden = hiddenStrats.has(strat);
            return (
              <div
                key={strat}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg border transition-all ${
                  isHidden ? "border-white/[0.04] bg-transparent opacity-40" : "border-amber-400/20 bg-amber-400/[0.06]"
                }`}
              >
                {strategies.length > 1 && (
                ...
                <span className="text-lg font-bold text-white ml-2">{total < 100 ? total.toFixed(2) : Math.round(total).toLocaleString()}</span>
                <span className="text-[10px] text-amber-400 font-medium">{unit}</span>
              </div>
            );
          })}
        </div>
